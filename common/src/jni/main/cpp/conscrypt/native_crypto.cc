...
// CRYPTO_BUFFER *ByteArrayToCryptoBuffer(JNIEnv* env, const jbyteArray array,
//                                        CONSCRYPT_UNUSED CRYPTO_BUFFER_POOL* pool) {
//     CRYPTO_BUFFER *ret = nullptr;
...
//     ret = CRYPTO_BUFFER_new(reinterpret_cast<const uint8_t*>(arrayRo.get()), arrayRo.size(),
//                             nullptr);
//     if (!ret) {
...
static jstring evp_print_func(JNIEnv* env, jobject pkeyRef, print_func* func,
    ...
    if (func(buffer, pkey, 0, nullptr) != 1) {
...
static jlong NativeCrypto_EVP_parse_private_key(JNIEnv* env, jclass, jbyteArray keyJavaBytes) {
    ...
    EVP_PKEY *pkey = d2i_AutoPrivateKey(NULL, &p, bytes.size());
    if (pkey == nullptr) {
    ...
    JNI_TRACE("bytes=%p EVP_parse_private_key => %p", keyJavaBytes, pkey);
...
static jobjectArray NativeCrypto_get_RSA_private_params(JNIEnv* env, jclass, jobject pkeyRef) {
    ...
    jbyteArray n = bignumToArray(env, RSA_get0_n(rsa), "n");
    if (env->ExceptionCheck()) {
        return nullptr;
    }
    env->SetObjectArrayElement(joa, 0, n);

    if (RSA_get0_e(rsa) != nullptr) {
        jbyteArray e = bignumToArray(env, RSA_get0_e(rsa), "e");
        if (env->ExceptionCheck()) {
            return nullptr;
        }
        env->SetObjectArrayElement(joa, 1, e);
    ...
    if (RSA_get0_d(rsa) != nullptr) {
        jbyteArray d = bignumToArray(env, RSA_get0_d(rsa), "d");
        if (env->ExceptionCheck()) {
            return nullptr;
        }
    ...
    if (RSA_get0_p(rsa) != nullptr) {
        jbyteArray p = bignumToArray(env, RSA_get0_p(rsa), "p");
        if (env->ExceptionCheck()) {
            return nullptr;
        }
    ...
    if (RSA_get0_q(rsa) != nullptr) {
        jbyteArray q = bignumToArray(env, RSA_get0_q(rsa), "q");
        if (env->ExceptionCheck()) {
            return nullptr;
        }
    ...
    if (RSA_get0_dmp1(rsa) != nullptr) {
        jbyteArray dmp1 = bignumToArray(env, RSA_get0_dmp1(rsa), "dmp1");
        if (env->ExceptionCheck()) {
            return nullptr;
        }
    ...
    if (RSA_get0_dmq1(rsa) != nullptr) {
        jbyteArray dmq1 = bignumToArray(env, RSA_get0_dmq1(rsa), "dmq1");
        if (env->ExceptionCheck()) {
            return nullptr;
        }
    ...
    if (RSA_get0_iqmp(rsa) != nullptr) {
        jbyteArray iqmp = bignumToArray(env, RSA_get0_iqmp(rsa), "iqmp");
        if (env->ExceptionCheck()) {
            return nullptr;
        }
...
//    ScopedByteArrayRW out(env, outArray);
//    if (out.get() == nullptr) {
//        JNI_TRACE("X25519(%p, %p, %p) can't get output buffer",
//                  outArray, privkeyArray, pubkeyArray);
//        return JNI_FALSE;
//    }
//
...
//        JNI_TRACE("X25519_keypair(%p, %p) can't get output public key buffer",
...
//        JNI_TRACE("X25519_keypair(%p, %p) can't get output private key buffer",
//                  outPublicArray, outPrivateArray);
//        return;
//    }
//
...
//                               evp_aead_ctx_op_func realFunc, jobject inBuffer, jobject outBuffer,
//                               jint outRange, jint inRange)  {
//    const EVP_AEAD* evpAead = reinterpret_cast<const EVP_AEAD*>(evpAeadRef);
//
//    ScopedByteArrayRO keyBytes(env, keyArray);
//    if (keyBytes.get() == nullptr) {
...
//    if (inCapacity == -1) {
//        conscrypt::jniutil::throwException(env, "java/lang/IllegalArgumentException",
//                                           "Non Direct ByteBuffer  Error");
//        return 0;
//    }
//    inBuf = reinterpret_cast<uint8_t*>(env->GetDirectBufferAddress(inBuffer));
...
//        return 0;
//    }
//    outBuf = reinterpret_cast<uint8_t*>(env->GetDirectBufferAddress(outBuffer));
//    // limit is the index of the first element that should not be read or written
...
static BIO_METHOD *create_bio_meth(void) {
    ...
    if (!BIO_meth_set_write(meth, bio_stream_write)
        || !BIO_meth_set_read(meth, bio_stream_read)
        || !BIO_meth_set_puts(meth, bio_stream_puts)
        || !BIO_meth_set_gets(meth, bio_stream_gets)
        || !BIO_meth_set_ctrl(meth, bio_stream_ctrl)
        || !BIO_meth_set_create(meth, bio_stream_create)
        || !BIO_meth_set_destroy(meth, bio_stream_destroy)) {
        BIO_meth_free(meth);
        return nullptr;
    }
...
//     if (gn_stack == nullptr) {
//         JNI_TRACE("get_X509_GENERAL_NAME_stack(%p, %d) => null (no extension or error)",
//                   x509, type);
//         ERR_clear_error();
//         return nullptr;
//     }
...
static jbyteArray get_X509Type_serialNumber(JNIEnv* env, const T* x509Type,
    ...
    BIGNUM *serialBn = ASN1_INTEGER_to_BN(serialNumber, nullptr);
    if (serialBn == nullptr) {
        JNI_TRACE("X509_get_serialNumber(%p) => threw exception", x509Type);
        return nullptr;
    }

    ScopedLocalRef<jbyteArray> serialArray(env, bignumToArray(env, serialBn, "serialBn"));
    if (env->ExceptionCheck()) {
        BN_free(serialBn);
        JNI_TRACE("X509_get_serialNumber(%p) => threw exception", x509Type);
        return nullptr;
...
static jbyteArray NativeCrypto_get_X509_tbs_cert_without_ext(JNIEnv* env, jclass, jlong x509Ref,
    ...
    ASN1_OBJECT *obj = OBJ_txt2obj(oid.c_str(), 1 /* allow numerical form only */);
    if (obj == nullptr) {
        X509_free(copy);
    ...
    int extIndex = X509_get_ext_by_OBJ(copy, obj, -1);
    if (extIndex == -1) {
        X509_free(copy);
    ...
    // cached encoding.
    X509_EXTENSION_free(X509_delete_ext(copy, extIndex));
    jbyteArray ret = ASN1ToByteArray<X509>(env, copy, i2d_re_X509_tbs);
...
static X509_EXTENSION* X509Type_get_ext(JNIEnv* env, const T* x509Type, jstring oidString) {
    ...
    ASN1_OBJECT *asn1 = OBJ_txt2obj(oid.c_str(), 1);
    if (asn1 == nullptr) {
    ...
    int extIndex = get_ext_by_OBJ_func(x509Type, asn1, -1);
    if (extIndex == -1) {
        ASN1_OBJECT_free(asn1);
...
static void NativeCrypto_ASN1_TIME_to_Calendar(JNIEnv* env, jclass, jlong asn1TimeRef,
    ...
    ASN1_GENERALIZEDTIME *gen = ASN1_TIME_to_generalizedtime(asn1Time, nullptr);
    if (gen == nullptr) {
    ...
    if (ASN1_STRING_length(gen) < 14 || ASN1_STRING_get0_data(gen) == nullptr) {
        ASN1_TIME_free(gen);
    ...
    const char* data = reinterpret_cast<const char*>(ASN1_STRING_get0_data(gen));
...
static jlong NativeCrypto_SSL_CTX_new(JNIEnv* env, jclass) {
    ...
    uint32_t mode = SSL_CTX_get_mode(sslCtx);
    /*
...
static jlong NativeCrypto_SSL_new(JNIEnv* env, jclass, jlong ssl_ctx_address,
    ...
    JNI_TRACE("ssl_ctx=%p NativeCrypto_SSL_new => ssl=%p appData=%p", ssl_ctx, ssl, appData);
    return (jlong)ssl;
}
...
static jstring NativeCrypto_SSL_CIPHER_get_kx_name(JNIEnv* env, jclass, jlong cipher_address) {
    ...
    return env->NewStringUTF(ssl_CIPHER_get_kx_name(cipher));
}
...
static jobjectArray NativeCrypto_get_cipher_names(JNIEnv* env, jclass, jstring selectorJava) {
    ...
    if (!SSL_set_cipher_list(ssl, selector.c_str())) {
        conscrypt::jniutil::throwException(env, "java/lang/IllegalArgumentException",
...
//     int result = EVP_PBE_scrypt(reinterpret_cast<const char*>(password_bytes.get()),
//                                 password_bytes.size(),
//                                 reinterpret_cast<const uint8_t*>(salt_bytes.get()),
//                                 salt_bytes.size(),
//                                 n, r, p, memory_limit,
...
//        CONSCRYPT_NATIVE_METHOD(X25519_keypair, "([B[B)V"),
        ...
        // TODO tongsuo no EVP_aead_*/EVP_AEAD_*, -> EVP_*/EVP_CTX_*
